    ...
    public function generate(array $filePaths, string $mode, string $pattern, string $replacement, bool $increment = false, string $incSep = ' - ', string $incFormat = '{name}{sep}{i}'): array {
        ...
        foreach ($filePaths as $index => $path) {
            ...
            if ($mode === 'metadata' && $path !== '' && $pattern !== '') {
                ...
                if ($target === 'name') {
                    $newName = str_replace($pattern, $replacement, $namePart) . $extPart;
                } elseif ($target === 'extension') {
                ...
                if ($target === 'name') {
                    ...
                    $newName = trim($newName, '_') . $extPart;
                } elseif ($target === 'extension') {
                ...
                if ($target === 'name') {
                    $newName = preg_replace('/\s+/u', '', $namePart) . $extPart;
                } elseif ($target === 'extension') {
                ...
                if ($target === 'name') {
                    $newName = mb_strtoupper(mb_substr($namePart, 0, 1)) . mb_substr($namePart, 1) . $extPart;
                } elseif ($target === 'extension') {
                ...
                if ($target === 'name') {
                    $newName = preg_replace_callback('/\b\w/u', function($m) { return mb_strtoupper($m[0]); }, $namePart) . $extPart;
                } elseif ($target === 'extension') {
